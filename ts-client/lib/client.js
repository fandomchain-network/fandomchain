...
export class IgniteClient extends EventEmitter {
    ...
    async useKeplr(keplrChainInfo = {}) {
        ...
        try {
            ...
            const denomsMetadata = await (await bankqc.queryDenomsMetadata()).data;
            const metadataMap = new Map(denomsMetadata.metadatas?.map((m) => [
                m.base,
                m.denom_units?.find((u) => u.denom === m.display)?.exponent ?? 0,
            ]) ?? []);
            let currencies = tokens.supply?.map((x) => {
                const coinDecimals = metadataMap.get(x.denom ?? "") ?? 0;
                const y = {
            ...
            const stakeDenom = staking.params?.bond_denom ?? "";
            const stakeCoinDecimals = metadataMap.get(stakeDenom) ?? 0;
            let stakeCurrency = {
                coinDenom: stakeDenom?.toUpperCase() ?? "",
                coinMinimalDenom: stakeDenom ?? "",
                coinDecimals: stakeCoinDecimals,
            };
