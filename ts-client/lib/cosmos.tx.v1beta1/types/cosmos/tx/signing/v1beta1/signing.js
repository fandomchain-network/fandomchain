...
export const SignatureDescriptor_Data = {
    encode(message, writer = new BinaryWriter()) {
        if (message.single !== undefined) {
            SignatureDescriptor_Data_Single.encode(message.single, writer.uint32(10).fork()).join();
        }
        if (message.multi !== undefined) {
            SignatureDescriptor_Data_Multi.encode(message.multi, writer.uint32(18).fork()).join();
        }
        return writer;
    },
    decode(input, length) {
...
export const SignatureDescriptor_Data_Multi = {
    encode(message, writer = new BinaryWriter()) {
        ...
        for (const v of message.signatures) {
            SignatureDescriptor_Data.encode(v, writer.uint32(18).fork()).join();
        }
        return writer;
    },
    decode(input, length) {
